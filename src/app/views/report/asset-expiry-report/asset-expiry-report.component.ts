...
@Component({
  selector: 'app-asset-expiry-report',
  templateUrl: './asset-expiry-report.component.html',
  styleUrls: ['./asset-expiry-report.component.scss']
})
export class AssetExpiryReportComponent implements OnInit {
  ...
  assetItemList:any=[];
  getAssetItemList(){
    this.reportService.getAssetItemList().subscribe(success=>{
      this.assetItemList = success;
   })  
  }

  selectedAssetItem(id){
    let tempList:any = this.assetItemList.filter(d=> d.x_asset_item_id == id);
    if(tempList){
      this.expiryreportform.get('amount').setValue(tempList[0].amount);
  
      this.expiryreportform.get('qty').setValue(tempList[0].qty);
  
      if(tempList[0].expiryDt){this.expiryreportform.get('expiryDt').setValue(this.commonService.dateFormat(tempList[0].expiryDt))} else{
        this.expiryreportform.get('expiryDt').setValue(null);
      };
  
      if(tempList[0].purchaseDt){this.expiryreportform.get('purchaseDt').setValue(this.commonService.dateFormat(tempList[0].purchaseDt))} else{
        this.expiryreportform.get('purchaseDt').setValue(null);
      };
  }
  }
// ==========================================
