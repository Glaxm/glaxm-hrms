...
@Component({
  selector: 'app-loan-history-report',
  templateUrl: './loan-history-report.component.html',
  styleUrls: ['./loan-history-report.component.scss']
})
export class LoanHistoryReportComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.moduleList) {
      this.moduleList.map((e) => {
        if (e.moduleGroup == 'Report' && e.moduleName == 'Loan/Advance History Report') {
  ...
  filterTable() {
    ...
    if (this.loanhistoryform.invalid) {
      ...
      this.getLoanReportSummary(data, this.loanhistoryform.value.stdate, this.loanhistoryform.value.endate);
  ...
  getLoanReportSummary(data, stdate, endate) {
    this.reportService.getLoanReportSummary(data, stdate, endate).subscribe(data => {
        ...
        this.filtered = Array(this.loanReportSummaryList.length).fill(0).map((x, i) => (
          {
            ...
            documentno: this.loanReportSummaryList[i].documentno,
            loanamount: this.loanReportSummaryList[i].loanamount,
            loandate: this.loanReportSummaryList[i].loandate,
            repaidamount: this.loanReportSummaryList[i].repaidamount,
            balance: this.loanReportSummaryList[i].balance,
            lEmploanId: this.loanReportSummaryList[i].lEmploanId,
  ...
  exportReport() {
    let title = "Loan_Advance_History_Report";
    let list = this.loanReportSummaryList;
    if (list) {
      let exportList = Array(list.length).fill(0).map((x, i) => (
        {

          "Company Name": list[i].companyName,
          "Emp. ID": list[i].oldEmpId,
          "Empname":list[i].empname,
          "Document No.": this.loanReportSummaryList[i].documentno,
          "Loan Amount": this.loanReportSummaryList[i].loanamount,
          "Start Date": this.loanReportSummaryList[i].startdate,
          "End Date": this.loanReportSummaryList[i].enddate,
          "Repaid Amount": this.loanReportSummaryList[i].repaidamount,
          "Balance": this.loanReportSummaryList[i].balance
